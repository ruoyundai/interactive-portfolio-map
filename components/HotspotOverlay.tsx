...
// Correctly importing the Hotspot interface from types.ts
...
const Typewriter: React.FC<{ text: string; delay?: number }> = ({ text, delay = 30 }) => {
  ...
  useEffect(() => {
    // Reset state when text changes to allow re-typing if needed
    setCurrentText('');
    setCurrentIndex(0);
  }, [text]);
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        ...
        >
          <div className="bg-black/90 backdrop-blur-xl border border-white/20 overflow-hidden shadow-2xl">
            {/* Header */}
            ...
            {/* Zoom Image Section */}
            <div className="relative h-40 border-b border-white/10 overflow-hidden bg-neutral-900">
              <div 
                className="absolute inset-0 scale-[1.1] origin-center transition-transform duration-700"
                style={{
                  backgroundImage: `url(${IMAGES.base})`,
                  backgroundPosition: `${hotspot.x + hotspot.width / 2}% ${hotspot.y + hotspot.height / 2}%`,
                  backgroundSize: '400%', 
                  backgroundRepeat: 'no-repeat'
                }}
              />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-transparent h-[200%] animate-[scanline_4s_linear_infinite] pointer-events-none" />
              <div className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] bg-[length:100%_2px,3px_100%] pointer-events-none" />
            </div>

            {/* Content Area - Typography adjusted to match screenshot */}
